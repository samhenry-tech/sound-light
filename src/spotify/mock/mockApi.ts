...
export function mockSearch(query: string): Promise<SpotifySearchResults> {
...
export function mockResolveSources(uris: readonly string[]): Promise<ResolvedSource[]> {
  const resolved = uris
...
export function mockResolveTracks(uris: readonly string[]): Promise<SpotifyTrack[]> {
  const resolved = uris
